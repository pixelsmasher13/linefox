...
pub fn save_activity_item(
    ...
    window_app_name, os_details, similarity_percentage_to_previous_ocr_text, keypress_count,
     full_activity_text, interval_length, editing_mode, element_tree_dump, detected_actions, original_ocr_text, automation_id)
    ...
    statement.execute(named_params! {
        ...
        "@automation_id": activity_item.automation_id,
    })?;
    Ok(())
}
    ...
    while let Some(row) = rows.next()? {
        activity_logs.push(ActivityItem {
           // id: row.get("id")?,
            ...
            automation_id: row.get("automation_id").ok(),
        });
    }
...
pub fn get_activity_logs_by_automation_id(db: &Connection, automation_id: i64) -> Result<Vec<ActivityItem>, rusqlite::Error> {
    let mut statement = db.prepare(
        "SELECT * FROM activity_logs 
         WHERE automation_id = ?
    ...
    let mut rows = statement.query([automation_id])?;
    let mut activity_logs: Vec<ActivityItem> = Vec::new();
